...
// Opciones de fondo de pantalla gestionadas en applySavedBackground()
...
const DOM = {
    ...
    bgPickerModal: document.getElementById('bgPickerModal'),
    bgOptionsContainer: document.getElementById('bgOptionsContainer')
};
...
function getExtUrl(baseName, suffix, exts, fallback) {
    // Retorna un array de strings `url('path')` para probar como fallback en CSS
    return exts.map(ext => `url('./images/${baseName}_${suffix}.${ext}')`).join(', ') + `, url('${fallback}')`;
}

function applySavedBackground() {
    const savedBgId = localStorage.getItem('church_custom_bg') || 'default_ai';
    
    if (savedBgId !== 'default_ai') {
        const exts = ['jpg', 'png', 'jpeg'];
        // Configurar CSS para que intente con jpg, luego png, luego jpeg. Si ninguna existe, fallback al default AI.
        const mobileUrls = getExtUrl(savedBgId, 'movil', exts, './images/lds_temple_day.png');
        const pcUrls = getExtUrl(savedBgId, 'pc', exts, './images/lds_temple_day.png');
        
        document.documentElement.style.setProperty('--bg-image-mobile', mobileUrls);
        document.documentElement.style.setProperty('--bg-image-desktop', pcUrls);
    } else {
        // Restaurar defaults (que usan los archivos lds_temple_day/night definidos en css)
        document.documentElement.style.removeProperty('--bg-image-mobile');
        document.documentElement.style.removeProperty('--bg-image-desktop');
    }
}

function createBgOptionNode(id, name, previewSrc, currentBg, allNodes) {
    const isSelected = currentBg === id;
    const opt = document.createElement('div');
    opt.dataset.bgId = id;
    opt.style.cssText = `cursor: pointer; border-radius: 8px; overflow: hidden; border: 3px solid ${isSelected ? 'var(--accent-gold)' : 'transparent'}; position: relative; transition: border-color 0.3s;`;
    ...
    label.style.cssText = "text-align: center; font-size: 0.75rem; padding: 4px; background: rgba(0,0,0,0.7); color: white; width: 100%; position: absolute; bottom: 0;";
    ...
    opt.addEventListener('click', () => {
        localStorage.setItem('church_custom_bg', id);
        applySavedBackground();
        // Actualizar bordes de todos los recuadros
        allNodes.forEach(n => {
            n.style.border = '3px solid transparent';
        });
        opt.style.border = '3px solid var(--accent-gold)';
    });
    
    allNodes.push(opt);
    return { node: opt, img: img };
}

function openBgPicker() {
    DOM.bgOptionsContainer.innerHTML = '';
    const currentBg = localStorage.getItem('church_custom_bg') || 'default_ai';
    const isMobile = window.innerWidth < 768;
    const suffix = isMobile ? 'movil' : 'pc';
    const allNodes = []; // Para actualizar los bordes al hacer clic
    
    // Opción automática (default)
    const defaultPreview = document.body.classList.contains('light-theme') 
        ? './images/lds_temple_day.png' 
        : './images/lds_temple_night.png';
    const defaultOpt = createBgOptionNode('default_ai', 'Automático (Día/Noche)', defaultPreview, currentBg, allNodes);
    DOM.bgOptionsContainer.appendChild(defaultOpt.node);
    
    // Generar opciones dinámicamente probando de 1 a 20
    for(let i = 1; i <= 20; i++) {
        const id = `temple_${i}`;
        const initialSrc = `./images/${id}_${suffix}.jpg`;
        const fallbackSrc = `./images/${id}_${suffix}.png`;
        
        const optData = createBgOptionNode(id, `Fondo ${i}`, initialSrc, currentBg, allNodes);
        
        // Lógica para ocultar la opción si la imagen no existe
        optData.img.onerror = function() {
            if (this.src.endsWith('.jpg')) {
                this.src = fallbackSrc; // Intentar con png
            } else {
                optData.node.style.display = 'none'; // Si también falla png, ocultar
            }
        };
        
        DOM.bgOptionsContainer.appendChild(optData.node);
    }
    
    if(DOM.bgPickerModal) DOM.bgPickerModal.classList.remove('hidden');
}
...
// Funciones de fondo eliminadas y reemplazadas por applySavedBackground()
        ...
        try {
            ...
            if (admins.length === 0) {
                ...
                admins.forEach((admin, index) => {
                    // Mostrar solo si es admin de distrito, o si pertenece a la rama actual (o si es legado sin rama)
                    if (admin.role !== 'district' && admin.role !== 'superadmin' && admin.wardName && admin.wardName !== state.wardName) {
                        return; // Saltar y no mostrar
                    }
...
function setupEventListeners() {
    ...
    window.addEventListener('scroll', () => {
        ...
        // Mover el fondo a un 20% de la velocidad de scroll
        document.documentElement.style.setProperty('--bg-scroll-offset', `${scrollY * 0.2}px`);
    });

    if (DOM.changeBgBtn) {
        DOM.changeBgBtn.addEventListener('click', openBgPicker);
    }
